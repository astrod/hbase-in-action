...
public class TwitsDAO {
	...
	public Twit getTwit(String user, DateTime dt) throws IOException {
	...
	public List<Twit> list(String user) throws IOException {

		Table table = connection.getTable(TableName.valueOf(TABLE_NAME));

		ResultScanner results = table.getScanner(mkScan(user));
		List<Twit> ret = new ArrayList<Twit>();
	...
	private static class Twit extends AbstractTwit {
		...
		private Twit(Result r) {
			this(
				r.getColumnLatestCell(TWITS_FAM, USER_COL).getRowArray(),
